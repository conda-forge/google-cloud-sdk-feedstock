...
for %%F in (gcloud.cmd gsutil.cmd bq.cmd docker-credential-gcloud.cmd) do (
    ...
    powershell -Command "$content = Get-Content '%outdir%\bin\%%F';$replacement = 'set CLOUDSDK_PYTHON=%PREFIX%\python.exe' + [Environment]::NewLine + 'set CLOUDSDK_ROOT_DIR=%outdir%'; $content -replace 'rem <cloud-sdk-cmd-preamble>', $replacement | Set-Content '%outdir%\bin\%%F'"
