...
for %%F in (gcloud.cmd gsutil.cmd bq.cmd docker-credential-gcloud.cmd) do (
    ...
    powershell -Command "(Get-Content '%outdir%\bin\%%F') -replace 'rem <cloud-sdk-cmd-preamble>', 'set CLOUDSDK_PYTHON=%PREFIX%\python.exe' + [Environment]::NewLine +'set CLOUDSDK_ROOT_DIR=%outdir%' | Set-Content '%outdir%\bin\%%F'"
